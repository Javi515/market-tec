...
class ProductDetailActivity : AppCompatActivity() {
    ...
    private val currentUserName: String = "Armando Hernández"
    ...
    private fun bindProductData() {
        ...
        if (!product.productImage.isNullOrEmpty()) {
            Glide.with(this).load(product.productImage)
        ...
        product.vendor?.let { vendor ->
            val fullName = buildString {
                append(vendor.firstName.orEmpty())
                if (!vendor.lastName.isNullOrEmpty()) {
                    append(" ")
                    append(vendor.lastName)
                }
            }.trim()

            tvVendorName.text = fullName.ifEmpty { "Vendedor desconocido" }
    ...
    private fun submitReview() {
        ...
        lifecycleScope.launch {
            try {
                ...
                if (response.isSuccessful) {
                    Toast.makeText(this@ProductDetailActivity, "¡Reseña enviada con éxito! ⭐", Toast.LENGTH_LONG).show()
                    etComment.text?.clear()
                    ratingBarInput.rating = 0f
                    loadReviews(product.id)
                } else {
                    Toast.makeText(this@ProductDetailActivity, "Error del servidor", Toast.LENGTH_SHORT).show()
                }
            } catch (e: Exception) {
    ...
    private fun showEditDialog(review: ReviewModel) {
        ...
        // La línea mágica que evita el crash en Android 14/15
        ratingBarEdit.post {
